...
Today, were going to build an application that will help us handle questions from students. To do this, we will set up the database and then overlay Ruby code to map the data from the database into Ruby objects in memory that we can work with. Our database queries (written in SQL) will live within our Ruby code.
...
SQL
Well first construct a series of tables. Write the table definitions in a SQL script named import_db.sql.
...
Dont forget to keep track of the body of a reply.
"Top level" replies dont have any parent, but all replies have a subject question.
Its okay for a column to be self referential; a foreign key can point to a primary key in the same table.
